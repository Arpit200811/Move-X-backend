    ...
    try {
        ...
        if (data.length === 0 && lang !== 'en') {
            ...
            const translations = await Promise.all(
                keys.map(key => TranslationService.translate(ENGLISH_DICTIONARY[key], lang))
            );
        ...
        if (missingKeys.length > 0) {
            ...
            const newTranslations = await Promise.all(
                missingKeys.map(key => lang === 'en' ? ENGLISH_DICTIONARY[key] : TranslationService.translate(ENGLISH_DICTIONARY[key], lang))
            );
...
export const seedTranslations = async (req: Request, res: Response) => {
    try {
        const translationRepository = AppDataSource.getRepository(Translation);
